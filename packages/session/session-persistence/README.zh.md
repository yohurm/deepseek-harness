...
后端读取会在返回记录前校验确切的当前格式记录。已废弃的 v0 形态——`steering/message`、pre-identity 消息事件、带 `trigger` 的 `turn/start` 以及已废弃的 `turn/end` 原因——会在共享追加边界与加载时被拒绝，并点名事件类型与 seq；同一遍校验当前格式 `turn/end` 的包络与原因。存储仍然仅追加：读取绝不重写记录。[会话旧形态终止注记](../../../.agents/notes/implemented/architecture/2026-08-21-session-legacy-shape-termination.md)拥有该决策；不提供任何 v0 升级路径。
