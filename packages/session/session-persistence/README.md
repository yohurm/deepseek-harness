...
Backend reads validate the exact current-format records before returning them. Retired v0 shapes — `steering/message`, pre-identity message events, a `turn/start` with a `trigger`, and retired `turn/end` reasons — refuse at the shared append boundary and at load, naming the event type and seq; current-format `turn/end` envelopes and reasons are validated in the same pass. Storage remains append-only: reads never rewrite records. The [session-legacy-shape-termination note](../../../.agents/notes/implemented/architecture/2026-08-21-session-legacy-shape-termination.md) owns the decision; no v0 upgrade path ships.
