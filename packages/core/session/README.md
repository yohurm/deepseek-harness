...
An interrupted live turn ends with `{ kind: 'aborted', reason: AgentCancelCause }`, preserving the typed cancellation cause in the durable transcript. A retired coarse aborted outcome without its caller refuses at the persistence boundary; the `legacy` cancel-cause variant no longer exists. A turn failure carries `{ kind: 'error', error }`; crash recovery alone synthesizes `{ kind: 'interrupted' }`.
