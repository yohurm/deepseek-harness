/**
 * DynamicCordisRunnerService: the Host-half registry, sandboxed activation,
 * run round trip, and error presentation behind ctx.dynamicCordisRunner.
 * @module @deepseek-ai/dsh-cordis-host-runner/runner
 */
...
import { ApprovalRequestId as brandApprovalRequestId, CordisDynamicPackageId as brandPackageId, CordisDynamicPluginId as brandPluginId, CordisDynamicPluginRunId as brandRunId } from './index.ts'
import type { Config, DynamicCordisSnapshotRow } from './index.ts'
...
export class DynamicCordisRunnerService extends TypertRemoteService {
  ...
  define(request: DynamicCordisDefineRequest): DynamicCordisDefineReceipt {
    ...
    if (request.plugin.kind === 'new') {
      ...
      const pluginId = brandPluginId(this.registry.mintPluginId(prefix))
      plugin = {
        pluginId,
    ...
    const packageId = brandPackageId(this.registry.mintPackageId())
  ...
  private createAttempt(plan: ActivationPlan): DynamicCordisRunAttempt {
    return {
      pluginRunId: brandRunId(this.registry.mintPluginRunId()),
